...
class CRService {
	...
	start() {
		...
		scheduler.scheduleJob("* * * * *", async () => {
			console.log((new Date()).toLocaleDateString() + " > 정보 수집 시작..");
			await that.getClanInfo().then(async(data) => {
				if (!data) {
					console.log((new Date()).toLocaleDateString() + " > 클랜 정보 수집 실패");
				} else {
					if (that.info.hasOwnProperty('donations')) {
						if (that.info['donations'] > data['donations']) {
							console.log((new Date()).toLocaleDateString() + " > 주간 기록 갱신");
							await util.promisify(fs.writeFile)(dataFiles.lastWeekRecord, JSON.stringify(that.info, null, 4));
						}
					}
					that.info = data;
					await util.promisify(fs.writeFile)(dataFiles.info, JSON.stringify(that.info, null, 4));
				}
			});

			await that.getClanWar().then(async(data) => {
				if (!data) {
					console.log((new Date()).toLocaleDateString() + " > 클랜전 정보 수집 실패");
				} else {
					that.war = data;
					await util.promisify(fs.writeFile)(dataFiles.war, JSON.stringify(that.war, null, 4));
				}
			});

			await that.getClanWarLog().then(async(data) => {
				if (!data) {
					console.log((new Date()).toLocaleDateString() + " > 클랜전 기록 정보 수집 실패");
				} else {
					let wars = [];
					...
					await util.promisify(fs.writeFile)(dataFiles.warlog, JSON.stringify(that.warlog, null, 4));
				}
			});

			console.log((new Date()).toLocaleDateString() + " > 정보 수집 완료");
		});
	}
